...
export default class ConfigPluginsSecurityUpdate extends Command {
  static summary = "Update plugins.security (replace with a JSON object)";

  static description =
    "Sets allowKinds, allowSourceTypes, allowIds, denyIds, strict, etc. Replaces the whole security block.";
  ...
  async run(): Promise<void> {
    const { flags } = await this.parse(ConfigPluginsSecurityUpdate);
    ...
    config.plugins ??= {};
    config.plugins.security = parsed as PluginSecurityPolicy;

    persistConfig(path, config);
    this.log(`Updated plugins.security (${path})`);
  }
}
