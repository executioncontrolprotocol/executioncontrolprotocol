...
export default class ConfigLoggersLoggerUpdate extends Command {
  static summary = "Update loggers.config.<loggerId> (replaces blob)";
  ...
  async run(): Promise<void> {
    const { args, flags } = await this.parse(ConfigLoggersLoggerUpdate);
    ...
    if (config.loggers.config[args.id] === undefined) {
      this.error(
        `No logger config "${args.id}". Use "ecp config loggers logger add" first.`,
        { exit: 1 },
      );
    }
    ...
    this.log(`Updated loggers.config.${args.id} (${path})`);
  }
}
