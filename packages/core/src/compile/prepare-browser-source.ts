const ECP_IMPORT_RE =
  /import\s*(?:type\s*)?\{[^}]*\}\s*from\s*["']@executioncontrolprotocol\/(?:core|browser)["']\s*;?\s*/g

const ECP_SIDE_EFFECT_IMPORT_RE =
  /import\s*["']@executioncontrolprotocol\/(?:core|browser)["']\s*;?\s*/g
...
export function prepareBrowserWorkflowSource(source: string): string {
  const stripped = source
    .replace(ECP_IMPORT_RE, "")
    .replace(ECP_SIDE_EFFECT_IMPORT_RE, "")
    .trimStart()
